...
<a href="/admin/fotos/create" class="btn btn-primary shadow mb-3"><i class="fas fa-plus mr-2"></i>Adicionar</a>
<div class="card shadow">
    <div class="card-body">
        <h4><i class="fas fa-filter mr-2"></i>Filtros</h4>        
        <form method="GET" action="/admin/fotos">
            <div class="row">
                <div class="form-group col-lg-9">
                    <label for="busca">Digite o nome da foto</label>
            ...
            <a href="/admin/fotos" class="btn btn-secondary shadow mr-3"><i class="fas fa-sync-alt mr-2"></i>Limpar filtros</a>
        </form>
        <hr>
        @if($amount != 0)
                ...
                <tbody>
                    @foreach($fotos as $foto)
                        ...
                        <tr class="table-danger">
                            <td class="table-img"><img class="img-fluid rounded" src="https://s3.us-east-1.amazonaws.com/bergard-teste/{{ $foto->url }}"></td>
                            <td>{{ $foto->nome }}</td>
                            <td>
                                <a href="/admin/fotos/{{ $foto->id }}" class="btn btn-primary shadow" data-toggle="tooltip" title="Editar">
                        ...
                        @else
                        <tr>
                            <td class="table-img p-1"><img class="img-fluid rounded" src="https://s3.us-east-1.amazonaws.com/bergard-teste/{{ $foto->url }}"></td>
                            <td>{{ $foto->nome }}</td>
                            <td>
                                <a href="/admin/fotos/{{ $foto->id }}" class="btn btn-primary shadow" data-toggle="tooltip" title="Editar">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <button type="button" class="btn btn-danger shadow" data-toggle="modal" title="Excluir" data-target="#modalDelete{{ $foto->id }}"> 
...
@foreach($fotos as $foto)
    @if($foto->status != 'EXCLUIDO')
        <!-- Modal DELETE -->
        <div class="modal fade" id="modalDelete{{$foto->id}}" tabindex="-1" role="dialog" aria-hidden="true">
